...
/**
 * schema.ts—the pure Zod schema for a topic, framework-free so it can be
 * imported both by the Astro content collection (src/content/config.ts) and by
 * standalone Node scripts (the ingestion validate/repro gates). Keep this file
 * free of any `astro:content` / `astro/loaders` imports: those pull in Astro
 * virtual modules that cannot resolve outside a Vite build.
 */
